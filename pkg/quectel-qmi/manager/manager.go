...
const (
	defaultUIMRecoverCooldown      = 30 * time.Second
	defaultServiceTimeoutThreshold = 5
	defaultServiceTimeoutWindow    = 10 * time.Minute
	defaultModemResetDedupWindow   = 1 * time.Second
	defaultModemResetQuietWindow   = 1 * time.Second
	defaultPostRegRefreshDelay     = 800 * time.Millisecond
	defaultPostRegRefreshTimeout   = 4 * time.Second
	defaultPostRegRefreshCooldown  = 2 * time.Second
	recoverBackoffBase             = 500 * time.Millisecond
	recoverBackoffMax              = 15 * time.Second
	recoverBackoffJitterRatio      = 0.2
)
...
func (m *Manager) enableRawIP(parent context.Context) error {
	...
	if isLinux && !kernelEnabled {
		...
		if err := os.WriteFile(sysfsPath, []byte("Y"), 0o644); err != nil {
